...
def construct_mask(mask_dict, backplane, target, planet, mask_desc,
    ...
    if type(target) == type(""):
        ...
        if not oops.Body.exists(body_name): return

        (masker, shadower, face) = mask_desc

        if "R" in masker and planet == "SATURN":
            mask = mask | backplane.where_in_back(target,
                                                  "SATURN_MAIN_RINGS").vals
        if "P" in masker:
            mask = mask | backplane.where_in_back(target, planet).vals
            if planet == "PLUTO":
                mask = mask | backplane.where_in_back(target, "CHARON").vals
        ...
        if not ignore_shadows:
          if "R" in shadower and planet == "SATURN":
            mask = mask | backplane.where_inside_shadow(target,
                                                       "SATURN_MAIN_RINGS").vals
          if "P" in shadower:
            mask = mask | backplane.where_inside_shadow(target, planet).vals
            if planet == "PLUTO":
                mask = mask | backplane.where_inside_shadow(target,
                                                            "CHARON").vals
