...
ALT_FORMAT_DICT = {
    ...
    ("sub_longitude",  "-180")  : ("-180", 1, 8, "%8.3f",  None,     -999.)}
...
def construct_excluded_mask(backplane, target, planet, mask_desc,
    ...
    if "R" in masker and planet == "SATURN":
        excluded |= backplane.where_in_back(target,
...
def ninety_percent_gap_degrees(n):
...
def get_range_mod360(values, alt_format=None):
    ...
    values = values.vals[values.antimask].ravel()
    ...
    if diff_max >= ninety_percent_gap_degrees(values.size):
        return range_mod360
    else:
        return complete_coverage
