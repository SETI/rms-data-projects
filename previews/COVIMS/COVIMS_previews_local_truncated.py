...
def FromFile(filename):
    ...
    # Organize by lines
    size_needed = core_lines * stride[2]
    if size_needed > buffer.size:
        buffer2 = np.zeros(size_needed, dtype=core_dtype)
        buffer2[:buffer.size] = buffer
        buffer = buffer2
    ...
    for arg in sys.argv[1:]:
      ...
      for f in files:
        try:
          ...
          if os.path.isfile(f):
              if f.endswith(".QUB") or f.endswith(".qub"):
                print f
                process1(f)
