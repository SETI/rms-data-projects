...
from VERSIONS_v2 import VERSIONS_PDS4_VS_PDS3

# This dictionary is keyed by the PDS4 basename. It returns a tuple:
#   (PDS3 full path, PDS4 version id, True if multiple PDS3 versions exist,
#                                     latest PD4 version id)

PDS3_FILEPATHS = {}
VERSIONED_FILEPATHS = set()

# For each versioned file, figure out the latest version
pds4_versions = {}
for (newname, version, oldpath) in VERSIONS_PDS4_VS_PDS3:
    pds4_versions[newname] = version

for (newname, version, oldpath) in VERSIONS_PDS4_VS_PDS3:
    latest_newname = newname.replace('-v1', '')
    latest_newname = latest_newname.replace('-v2', '')
    latest_newname = latest_newname.replace('-v3', '')
    if latest_newname == newname:
        PDS3_FILEPATHS[newname] = (oldpath, version, version)
    else:
        PDS3_FILEPATHS[newname] = (oldpath, version,
                                   pds4_versions[latest_newname])

    VERSIONED_FILEPATHS.add(oldpath)    # Keep track of PDS3 paths with versions

# Now fill in the rest of the files
with open('PDS3_FILES.txt') as f:
    paths = f.readlines()

for path in paths:
    path = path.rstrip()
    if path in VERSIONED_FILEPATHS: continue
    ...
    PDS3_FILEPATHS[newname] = (path, '1.0', '1.0')
...
#     buffer = buffer.replace('\r','')
    recs = buffer.split('\r\n')
    recs = [r for r in recs if not r.startswith('|')]   # remove comments
    buffer = '\n'.join(recs)
    ...
    (lookup['pds3_filepath'],
     lookup['version'],
     lookup['latest_version']) = PDS3_FILEPATHS[os.path.basename(datafile)]
