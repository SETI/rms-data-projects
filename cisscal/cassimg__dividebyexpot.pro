...
IF ( err NE 0 ) THEN BEGIN
  IF DebugFlag gt 0 THEN CISSCAL_Log, '  No shutter offset file found: using nominal exposure time'
...
; Also need to develop way to subtract constant offset discussed in
; section 4.3 of Ground Calibration Report. Right now just use 1 ms
; for all cases.
; CORRECTION: New analysis of Vega images (through S03) points to a
; value more like 2.85 ms (correct to within about 0.05 ms for the
; NAC, less certain for the WAC. Use this until better data
; available. (12/1/2005 - BDK)
; UPDATE: Used azimuthal ring scans to pin down WAC to around 1.8 ms.
; (1/18/2006 - BDK) 
; UPDATE #2: S58 SPICA data gives WAC shutter offset of about 
; 2.86 ms. 
; (9/21/2010 - BDK)
...
;  ConstOffset = 0.0
;  IF self.Instrument EQ 'ISSNA' THEN ConstOffset = 2.85 ELSE ConstOffset = 1.8
;  IF self.Instrument EQ 'ISSNA' THEN ConstOffset = 2.85 ELSE ConstOffset = 2.86
...
;newhistory='Divided by exposure time'
