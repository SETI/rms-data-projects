...
;      if camera eq 'NAC' then begin   ; according to Bob, this is correct
;;      if camera eq 'NAC' and sum eq 'FULL' then begin
;          bdpath = CalibrationBaseDir + 'darkcurrent/nac_bias_distortion.tab'
;          cisscal_readspec,bdpath,samp,bias_distortion,nsamps          
;          for j = 0,numlines-1 do dark_e[0:20,j] = dark_e[0:20,j] - bias_distortion
;          stop
;      endif
...
;	The hardest part is probably the choice of a dark image to subtract...
;	therefore this is now done elsewhere (e.g. manually in the GUI)
;	and we just expect to find the image name in the DarkFName parameter
;
;	With an appropriate dark frame subtracted, this will clear
;	out spatial darkcurrent structure and RBI (Residual Bulk Image)
;
...
;CISSCAL_Log, 'Total before dark Subtraction: ',total (*self.ImageP)
...
;temp = self.Labels -> Get('SENSOR_HEAD_ELEC_TEMPERATURE')
...
; if in batch mode:
   ...
   if file_test(DarkFName) then begin
     ...
     ; correct for summation - bad idea? 
...
; CISSCAL DARK MODEL:
   ...
   ; update 'YDs' as new parameter files are aded:

   if camera eq 'ISSNA' then begin
      ESatPoint = 1.237e5     ; from values in params file
      cam = 'NAC'
;        dparamfile='nac_derived_dark_parameters04228.xdr'
   ...
   if camera eq 'ISSWA' then begin
      ...
      cam = 'WAC'       
;      dparamfile='wac_derived_dark_parameters04222.xdr'
   ...
   subscript = YFs[yearindex]
                                ...
                                ; (this line won't work for
                                ; parameter files made from
                                ; pre-2000 data; probably not an issue:)
;   YFs = float('20'+strmid(YDs,0,2)+float(strmid(YDs,2))/365.25)
; 
;   YearIndex = (where(abs(YFs-ImgTimeYF) eq min(abs(YFs-ImgTimeYF))))[0]
;   subscript = YDs[YearIndex]
;
;   dparamfile = strlowcase(cam) + '_median_dark_parameters' + subscript + '.xdr'    
   ...
   endelse                      ; which lack a binary header
    ...
    if havedark eq 0 then begin
       ...
       IF DebugFlag gt 0 THEN BEGIN ; did not find file, so create new
          CISSCAL_Log, dmsg
...
;      CISSCAL_Log, 'Total after dark Subtraction: ',total (*self.ImageP)
  ENDIF
