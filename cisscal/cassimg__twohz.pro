...
;;  Daren Scot Wilson,  December 23, 2003
...
; Email darenw@ciclops.org or darenw@pipeline.com
;
; just in case img is taken from an img set array,
...
;    tmpimg = smooth(tmpimg, 3, /edge_truncate)    ; before smoothing

   ; ignore first 12 and last 5 pixels of each line - may be abnormal
   ; in some operational modes.
        
;      Doesn't work in IDL 5.4:
;	hum = median(tmpimg[12:self.NS-13,*], dimension=1)
    hum = fltarr(self.NL)
...
;; DEFAULT FILTER PARAMETERS - found to be good over wide variety of images
;fwidth = 10.0
;order = 12
;
;; Create double-height hum signal
;h= self.NL ; height of image
;hum2 = fltarr(2*h)   ; extended hum signal to impose good boundary conditions
;hum2[0:h-1] = hum
;hum2[h:2*h-1] = reverse(hum)
;
;; Take Fourier of our 2Hz hum (including bias, dark current etc)
;fhum=shift(fft(hum2), h)
;
;; Create filter kernel in frequency space
;; note use of double precision to avoid overflow complaints
;iii = findgen(2*h)
;tf = 1.0 - 1.0/sqrt(1.0 + ((iii-(double(h)))/(2.*fwidth) < 100.)^(2*order))
;
;;original:
;;tf=  1.0 -  1.0/ sqrt((1.0 + ( (iii-(h/1.0D0)) / (2.*fwidth) < 100.) ^ (2*order) )  )
;
;; Apply filter and de-Fourier the signal
;fhum = temporary(tf*fhum)
;hum2 = float(fft(shift(fhum,h), /inverse))
;; float() extracts  the real part of a complex number
;
;hum = hum2[0:h-1]
