...
;	Choose a nominal optics temp name as per ISSCAL
  ...
  IF ( err NE 0 ) THEN BEGIN
    If DebugFlag gt 0 THEN CISSCAL_Log, $
      '  Flat database not found: unable to find ' + SlopeBaseDir+SlopeDBName
  ENDIF ELSE BEGIN
...
;      Fields = STR_SEP(LineBuf, ' ')	; tokenize
...
;	Classic IDL here: fetch slope image, rebinned to substantive image size
;	(by averaging)
;	I *don't* adjust here for changed pixel size in SUM2 and SUM4 modes
...
;	Need to correct for the gain state of the slope file
;	 using these relative gain values (slope always at gain 2)
;	Not done yet until I am convinced of which sense to apply
;      RelGainNA_5  = [ 7.70, 3.27, 1.00, 0.425 ]
;      RelGainNA_25 = [ 7.54, 3.41, 1.00, 0.478 ]
;      RelGainWA_5  = [ 7.61, 3.07, 1.00, 0.428 ]
;      RelGainWA_25 = [ 7.03, 3.26, 1.00, 0.424 ]

;       The following was cleaned up by BDK, 6/16/2009. All slope
;       files used by CISSCAL already 1024x1024, but we want to 
;       rebin to image size before normalizing:

;      Flatfield = temporary(median(SlopeImg[312:711,312:711])/SlopeImg)
;      Flatfield= Rebin(Flatfield,self.NS,self.NL)
...
;	Finished with the slope object, so eradicate it!
...
;newhistory='Divided by flat field'
