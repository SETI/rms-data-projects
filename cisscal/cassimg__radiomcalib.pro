...
PRO CassImg::RadiomCalib, Debug=Debug
...
;	The Debug keyword parameter provokes verbose logging of progress
...
IF self.Calibrated NE 0 THEN BEGIN
    IF DebugFlag gt 0 THEN BEGIN
        CISSCAL_Log 
        CISSCAL_Log, '**'
        CISSCAL_Log, '** Image already calibrated: calibration stage flag is ', $
                     self.Calibrated
        CISSCAL_Log, '**'
    ENDIF
    ...
    answer = dialog_message(message, /cancel, /default_cancel,$
                            dialog_parent=GuiBase, /information)  
    
    if answer ne 'OK' then return
ENDIF
...
;;	Add new PROCESSING_HISTORY_TEXT keyword to image header:
;junk=self.Labels->Set('PROCESSING_HISTORY_TEXT','',1,/new)
