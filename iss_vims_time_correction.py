...
def cassini_mission_name(image_mid_time, mission_data, mission_phase_name):
    ...
    """
    image_mid_time_text = image_mid_time.text

    # Try parsing as ISO 8601 (YYYY-MM-DDTHH:MM:SS.sssZ)
    try:
        image_dt = datetime.strptime(image_mid_time_text, "%Y-%m-%dT%H:%M:%S.%f")
    except ValueError:
        # Try parsing as Year-Day-of-Year (YYYY-DDDTHH:MM:SS.sss)
        try:
            image_dt = datetime.strptime(image_mid_time_text, "%Y-%jT%H:%M:%S.%f")
        except ValueError:
            print(f"Warning: Unable to parse image_mid_time {image_mid_time_text}. Keeping original value.")
            return
    ...
    print(f"Warning: No mission phase found for time {image_mid_time_text}. "
...
def start_time_replace(directory_path, pattern, kernel_dir, mission_data):
    ...
    for file_path in files:
        ...
        exposure_duration_elem = root.find(".//cassini:exposure_duration",
                                           namespaces=namespaces)
        mission_phase_name = root.find(".//cassini:mission_phase_name",
                                       namespaces=namespaces)
        image_mid_time = root.find(".//cassini:image_mid_time", namespaces=namespaces)
        image_time = root.find(".//cassini:image_time", namespaces=namespaces)
        ...
        image_time.text = new_stop_time.strftime("%Y-%jT%H:%M:%S.%f")[:-3]
        ...
        start_time_doy.text = new_start_time_dt.strftime("%Y-%jT%H:%M:%S.%f")[:-3]

        mid_time_dt = new_start_time_dt + (stop_time_dt - new_start_time_dt) / 2
        image_mid_time.text = mid_time_dt.strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3]
