...
def write_rec(f, label_filename, volume_id):
    ...
    f.write('"%-5s",' % label['FILTER_NAME'])
    f.write('"%-5s",' % label.get('DETECTOR_ID', 'N/A'))
    key = 'IMAGE' if 'IMAGE' in label else 'EXTENSION_QUALITY_IMAGE'
    f.write('%4d,'    % label[key]['LINE_SAMPLES'])
    f.write('%5d'     % label[key]['LINES'])
