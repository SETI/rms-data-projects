...
def read_tol_file(file = TOL_FILE):
    ...
    for i in range(1, number_of_events):
        assert start_times[i] >= start_times[i-1], event_names[i]
...
def find_event(tai, tol_list):
    ...
    # TAI too large
        ...
        loc = np.searchsorted(stop_times, tai)
        if loc < number_of_events:
            return event_names[loc-1]

        print "Time out of range. Final activity returned."
