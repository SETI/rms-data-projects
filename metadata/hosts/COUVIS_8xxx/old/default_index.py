...
ROOT = '/Volumes/pdsdata-offsite/holdings/volumes/COUVIS_8xxx/COUVIS_8001/data/'
INDEX = '/Volumes/pdsdata-offsite/holdings/volumes/COUVIS_8xxx/COUVIS_8001/index/'

f = open(INDEX + 'index.tab', 'w', encoding='latin-1')
...
for filename in filenames:
    ...
    reclist.append('"%-41s"' % (filename[:-4] + '.TAB'))
    reclist.append('"%-3s"'  % label['ORBIT_NUMBER'])
    reclist.append('"%-21s"' % label['START_TIME'])
    reclist.append('"%-21s"' % label['STOP_TIME'])
    reclist.append('"%-1s"'  % label['RING_OCCULTATION_DIRECTION'][0])
    reclist.append('"%-1s"'  % label['PLANETARY_OCCULTATION_FLAG'][0])
    reclist.append('"%-10s"' % label['STAR_NAME'])
    reclist.append('%5.1f'   % label['MINIMUM_WAVELENGTH'])
    reclist.append('%5.1f'   % label['MAXIMUM_WAVELENGTH'])
    reclist.append('"%-16s"' % label['SPACECRAFT_CLOCK_START_COUNT'])
    reclist.append('"%-16s"' % label['SPACECRAFT_CLOCK_STOP_COUNT'])
    reclist.append('"%-21s"' % label['RING_EVENT_START_TIME'])
    reclist.append('"%-21s"' % label['RING_EVENT_STOP_TIME'])
