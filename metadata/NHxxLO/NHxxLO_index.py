...
##########################################################################################
"""\
This program re-formats the content of a NHxxLO index file in a standard, consistent
format for the RMS node's PDS3 holdings/metadata tree.

To use:
    > python [--nomerge] NHxxLO_index.py path/to/index.tab ...
...
VOLUME_ID_REGEX = re.compile(r'(.*?)(NH[A-Z][A-Z0-9]LO_[12]001)')
...
LENGTHS = [11, 21, 31, 43, 27, 19, 10, 18, 18, 23, 23, 35, 12, 6 , 61, 7]
...
def write_nhxxlo_index(filepath, merge_filespec):
    ...
    label_nhxxlo_index(output_file, info)

    return output_file


NHXXLO_ORDER = {'LA':0, 'JU':1, 'PC':2, 'PE':3, 'KC':4, 'KE':5, 'K2':6}

def write_nhxxlo_cum_index(filepath, merge_filespec):
    ...
    # Identify the output file and optional backup
    index_pattern = prefix + f'NH??LO_{group}001/NH??LO_{group}001_index.tab'
    output_file = prefix + f'NHxxLO_{group}999/NHxxLO_{group}999_index.tab'

    index_files = glob.glob(index_pattern)
    index_files.sort(key=lambda k: NHXXLO_ORDER[os.path.basename(k)[2:4]])
    ...
    # Close and label the output file
    f.close()
    label_nhxxlo_index(output_file, {'merge_filespec': merge_filespec})

    return output_file


def label_nhxxlo_index(index_file, info):
    ...
    template_path = os.path.split(program_path)[0] + '/NHxxLO_index_template.txt'
...
if __name__ == '__main__':
    ...
    for filepath in sys.argv[1:]:
        print('\n' + filepath)
        index_file = write_nhxxlo_index(filepath, merge_filespec)
        index_files.append(index_file)

    # Generate cumulative indices by group, if necessary
    saved = {1: False, 2: False}
    for index_file in index_files:
        group = 1 if 'LO_1' in index_file else 2
        if saved[group]:
            continue
        print(f'\nmerging MHxxLO_{group}999_index_template.tab')
        write_nhxxlo_cum_index(index_file, merge_filespec)
        saved[group] = True
